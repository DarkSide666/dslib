<?php
namespace dslib;
class View_Chooser extends \View {
	public $crud;
	public $field;
	public $buttonset;
	
	public $crud_class='CRUD';
	public $field_class='Form_Field_Line';
	public $buttonset_class='Buttonset';
	...
	function init(){
		...
		// if dialog window enabled, initialize CRUD, cut it and return
		if(isset($_GET[$this->name])){
			$this->api->stickyGET($this->name);
			
			$this->crud = $this->add($this->crud_class);
			$_GET['cut_object'] = $this->name;
			return;
		}
		
		// else initialize text field + buttonset
		//$this->field = $this->add($this->field_class); // this doesn't work probably because no form set...
		//$this->buttonset = $this->add($this->buttonset_class);
	}
